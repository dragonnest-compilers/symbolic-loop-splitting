    ...
    struct LoopSplitting : public FunctionPass {
        ...
        Value *calculateIntersectionPoint(const SCEVAddRecExpr *first,
            ...
            if (predicate == CmpInst::ICMP_UGT || predicate == CmpInst::ICMP_SGT ||
                ...
                const auto one = ConstantInt::get(Type::getInt32Ty(*context), 1);
                const auto divisible = builder->CreateICmp(CmpInst::ICMP_NE, rem, zero, "divisible"); // rem == 0 ? 0 : 1
                const auto select = builder->CreateSelect(divisible, one, zero);
                const auto extended =
                builder->CreateZExt(select, IntegerType::getInt32Ty(*context),
        ...
        Value *calculateIntersectionPoint(const SCEVAddRecExpr *add,
            ...
            const auto b = getValueFromSCEV(add->getOperand(0));
            const auto a = getValueFromSCEV(add->getOperand(1));
        ...
        void splitLoop(Function *F, Loop *loop, LoopInfo *loopInfo,
            ...
            const SCEV *sc0 = SE->getSCEV(Cmp->getOperand(1));
            const SCEV *sc1 = SE->getSCEV(Cmp->getOperand(0));
            Value *SplitPoint;
            Value *y;
            Value *isLess;
            if (dyn_cast<SCEVAddRecExpr>(sc0) && dyn_cast<SCEVAddRecExpr>(sc1)) {
                ...
                const SCEVAddRecExpr *sc1Add = dyn_cast<SCEVAddRecExpr>(sc1);
                 ...
                 SplitPoint = calculateIntersectionPoint(sc0Add, sc1Add, predicate,
                ...
                isLess = builder.CreateICmp(CmpInst::ICMP_SLT, firstBefore, secondBefore);
            } else if ((dyn_cast<SCEVAddRecExpr>(sc0) && dyn_cast<SCEVConstant>(sc1)) ||
                       (dyn_cast<SCEVConstant>(sc0) && dyn_cast<SCEVAddRecExpr>(sc1))) {
                
                const SCEVAddRecExpr *scAdd;
                const SCEVConstant *scConstant;
                if (dyn_cast<SCEVAddRecExpr>(sc0)) {
                    scAdd = dyn_cast<SCEVAddRecExpr>(sc0);
                    scConstant = dyn_cast<SCEVConstant>(sc1);
                } else {
                    scAdd = dyn_cast<SCEVAddRecExpr>(sc1);
                    scConstant = dyn_cast<SCEVConstant>(sc0);
                }
                
                SplitPoint = calculateIntersectionPoint(scAdd, scConstant, predicate, &builder, &F->getContext());
                
                const auto a = getValueFromSCEV(scAdd->getOperand(1));
                ConstantInt *c = cast<ConstantInt>(a);
                errs() << c->getValue() << "\n";

                const auto b = getValueFromSCEV(scAdd->getOperand(0));
                ConstantInt *d = cast<ConstantInt>(b);
                errs() << d->getValue() << "\n";

                const auto mul = builder.CreateMul(a, SplitPoint);
                y = builder.CreateAdd(mul, b, "y");

                const auto one = ConstantInt::get(Type::getInt32Ty(F->getContext()), 1);
                const auto before = builder.CreateSub(SplitPoint, one);
                const auto beforeSCEV = SE->getSCEV(before);
                auto firstBefore = getValueFromSCEV(scAdd->evaluateAtIteration(beforeSCEV, *SE));

                auto secondBefore = scConstant->getValue();
                isLess = builder.CreateICmp(CmpInst::ICMP_SLT, firstBefore, secondBefore);
                ConstantInt *e = cast<ConstantInt>(isLess);
                errs() << e->getValue() << "\n";
